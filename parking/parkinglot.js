...
const sortingCars = () => {
  if (cars.length !== 0) {
    cars.sort(function (a, b) {
      return b.arrival.getTime() - a.arrival.getTime();
    });
  }
};
...
const countAvailablePlaces = () => {
  if (cars.length === 0) {
    document.querySelector("#placesCount").innerHTML = totalPlaces;
  } else {
...
const makeBill = (car) => {
  ...
  let billValue = car.isParked
    ? "-"
    : hoursBilled > 1
    ? "Rp " + (payFirstHour + hoursBilled * payPerHour)
    : "Rp " + payFirstHour;
  return billValue;
};
...
const checkLicensee = (newLicensee) => {
  let res = false;

  cars.map((data) => {
    if (data.licensee === newLicensee) {
      res = true;
    }
  });
...
const addCar = () => {
  ...
  let parked = checkLicensee(newLicensee);

  // Add new car to the cars array
  document.querySelector("#message").style.display = "none";
  if (
    newLicensee.length >= minLicenseeLength &&
    cars.length < totalPlaces &&
    !parked
  ) {
    cars.push(newCar);
    ...
    if (newLicensee.length < minLicenseeLength || parked) {
